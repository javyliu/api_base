class StatActiveFeeDay < PipstatRecord
  ...
  def self.get_fee_active(sdate, edate, gids, by_date: false)
    ...
    result = select(selects).by_gameid(gids).by_statdate(sdate, edate).group(groups).to_a
    ...
    result = result.group_by{|item| item.gamecode}
    p "---------------"
    p result
