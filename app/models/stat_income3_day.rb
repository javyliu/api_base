class StatIncome3Day < PipstatRecord
  ...
  def self.get_income(sdate,edate,gids)
    select("gamecode, sum(amount) as amount, sum(amount2) as amount2").by_gameid(gids).by_statdate(sdate,edate).group(:gamecode).map{|item| [item.gamecode, item.amount/100.0, item.amount2/100.0]}
  end
