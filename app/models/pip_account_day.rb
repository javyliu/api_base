...
class PipAccountDay < PipstatRecord
  ...
  def self.get_new_players(sdate,edate,gids, by_date: false)
    ...
    unless by_date
      ...
      players2 = old_gids.presence && StatAccountDay.select("gameswitch as gamecode, sum(reguser) as new_user").by_gameid(old_gids).by_statdate(sdate,edate).group(:gamecode).to_a || []
      result += players2
    end
